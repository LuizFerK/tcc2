...
for independent replication~\cite{pereira2023mulletbench}. The others either omitted
configurations entirely, described their setups in prose too vague to follow mechanically,
or used toolchains that require institutional access~\cite{shah2022performance,naqvi2017influxdb,
...
appearing in four of the five performance studies reviewed~\cite{naqvi2017influxdb,
...
the two most recent comparative studies~\cite{pereira2023mulletbench,wang2023iotdb},
...
capability without abandoning the relational ecosystem. It appears in three of the five
reviewed benchmarks~\cite{shah2022performance,lima2024avaliando,wang2023iotdb}.

Plain PostgreSQL without the TimescaleDB extension was explicitly excluded. TimescaleDB
supersedes it for this domain: any conclusion drawn from TimescaleDB is directly applicable
to a PostgreSQL deployment, and including both would duplicate tests without producing
additional insight.
...
University, which ships purpose-built Java clients for all three databases under evaluation.
...
overhead. iot-benchmark's uniform interface and IoT-specific workload semantics ---
concurrent clients organized into device and sensor hierarchies, configurable batch sizes,
and Poisson-distributed out-of-order data generation --- made it the most appropriate choice
for this study.
...
\begin{table}[ht]
  ...
  \caption{Benchmark scale parameters. All scales use a default batch size of 100 rows per
           request. Large scale contains ${\sim}100\times$ more data than small.}
  \label{tab:scales}
...
Large scale contains approximately 20$\times$ more data than medium and 100$\times$ more
...
\textbf{Nix flake}~\cite{nixos} provides a declarative, lock-file-pinned development
environment. Running \texttt{nix~develop} drops into a shell with exactly-specified versions
of Java~17, Maven, Python~3, and Docker tooling. Running \texttt{nix~run~.\#build} compiles
all three Java benchmark clients in a single step. Every transitive build dependency is
recorded in the flake lock file, eliminating the environment-variation failures that
typically plague benchmark replications.
...
the Java client's \texttt{conf/config.properties}, invokes the Java client, polls
...
hundreds of points, a latest-point query returns exactly one --- \textit{latency} is used
as the primary read comparison metric throughout Section~\ref{sec:experiments}.
...
\begin{enumerate}
  ...
  containers competed for OS page cache and I/O bandwidth. TimescaleDB accumulated several
