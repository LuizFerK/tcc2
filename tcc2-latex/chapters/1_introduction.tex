...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\columnwidth]{figures/projecao-iot}
...
of timestamped sequential data exposed the fundamental limitations of traditional relational
...
from the ground up to meet the unique demands of temporal data~\cite{InfluxDataTimeSeries}.
...
and economical resource usage~\cite{Dunning2014}.

The TSDB landscape has diversified considerably in recent years, with systems adopting
radically distinct architectural approaches to the same problem. \textit{Apache IoTDB}~\cite{wang2023iotdb},
...
\textit{InfluxDB} is the most widely benchmarked TSDB in the recent
literature~\cite{naqvi2017influxdb,shah2022performance,lima2024avaliando}, using a
Time-Structured Merge Tree (TSM) engine exposed via an HTTP line-protocol API. \textit{TimescaleDB}
extends PostgreSQL with time-series capabilities~\cite{Ramakrishnan2003}, offering SQL compatibility
...
allow independent replication~\cite{pereira2023mulletbench,shah2022performance,naqvi2017influxdb}.
...
parameterized Java benchmark clients --- publicly available alongside the results.
...
in the two most recent studies~\cite{pereira2023mulletbench,wang2023iotdb}, and TimescaleDB as
...
\begin{itemize}
  ...
  spanning 100$\times$ in data size;
