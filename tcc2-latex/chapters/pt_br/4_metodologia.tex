...
suficientes para replicação independente~\cite{pereira2023mulletbench}. Os demais ou omitiram
...
institucional~\cite{shah2022performance,naqvi2017influxdb,lima2024avaliando,wang2023iotdb}.
...
aparecendo em quatro dos cinco estudos de desempenho revisados~\cite{naqvi2017influxdb,
...
estudos comparativos mais recentes~\cite{pereira2023mulletbench,wang2023iotdb}, representando
...
a capacidade de séries temporais sem abandonar o ecossistema relacional. Aparece em três dos
cinco benchmarks revisados~\cite{shah2022performance,lima2024avaliando,wang2023iotdb}.

O PostgreSQL simples, sem a extensão TimescaleDB, foi explicitamente excluído. O TimescaleDB o
supera para este domínio: qualquer conclusão extraída do TimescaleDB é diretamente aplicável a
uma implantação PostgreSQL, e incluir ambos duplicaria os testes sem produzir informações
adicionais.
...
Universidade Tsinghua, que vem com clientes Java específicos para os três bancos de dados em
...
\begin{table}[ht]
  ...
  \caption{Parâmetros de escala do benchmark. Todas as escalas usam um tamanho de lote padrão
    de 100 linhas por requisição. A escala grande contém ${\sim}100\times$ mais dados
  que a pequena.}
  \label{tab:scales}
...
A escala grande contém aproximadamente 20$\times$ mais dados que a média e 100$\times$ mais
...
\textbf{Nix flake}~\cite{nixos} fornece um ambiente de desenvolvimento declarativo com arquivo
de lock fixado. Executar \texttt{nix~develop} inicia um shell com versões exatamente especificadas
de Java~17, Maven, Python~3 e ferramentas Docker. Executar \texttt{nix~run~.\#build} compila
todos os três clientes Java de benchmark em uma única etapa. Toda dependência transitiva de
construção é registrada no arquivo de lock do flake, eliminando as falhas de variação de
ambiente que tipicamente prejudicam replicações de benchmark.
...
\texttt{conf/config.properties} do cliente Java, invoca o cliente Java, consulta
...
\begin{enumerate}
  \item \textbf{Ciclo de vida sequencial de contêineres.} A configuração original iniciava todos
    ...
    largura de banda de E/S. O TimescaleDB acumulou vários gigabytes de cache de páginas durante
