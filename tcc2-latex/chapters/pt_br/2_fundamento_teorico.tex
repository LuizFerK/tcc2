...
dados~\cite{Ramakrishnan2003}. A necessidade de tais sistemas cresceu rapidamente, pois a
...
que mesmo usuários não especialistas conseguem entender o conteúdo armazenado~\cite{Ramakrishnan2003}.
...
Os bancos de dados distribuídos (BDDs) representam a convergência de duas tecnologias
aparentemente opostas: sistemas de banco de dados e redes de computadores~\cite{Ozsu2011}.
Enquanto os sistemas de banco de dados tradicionais promovem a centralização para controle e
integridade, as redes de computadores incentivam a distribuição e a descentralização. A tecnologia
dos BDDs sintetiza esses conceitos, demonstrando que a integração de dados não implica
necessariamente centralização física.
...
logicamente inter-relacionados, distribuídos por uma rede de computadores~\cite{Ozsu2011}.
...
ponto-a-ponto e sistemas multibanco~\cite{Ozsu2011}. Em sistemas cliente/servidor, as funções
...
cada um associado a um carimbo de tempo específico. De acordo com~\cite{Ozsu2011}, uma série
...
e muitos outros tipos de dados analíticos~\cite{InfluxDataTimeSeries}.
...
\begin{figure}[ht]
  ...
  Cada linha corresponde a uma medição com carimbo de tempo.}
  \label{fig:tsdb_record}
\end{figure}

A Figura~\ref{fig:tsdb_record} mostra a tabela correspondente em um BDST, onde cada linha é um
registro com carimbo de tempo que captura pressão atmosférica, temperatura, umidade e outras
...
é agora gerado a cada segundo~\cite{Dunning2014}. Essa explosão no volume de dados exige novas
...
para um intervalo de tempo específico sejam particularmente eficientes~\cite{Dunning2014}.
...
essenciais~\cite{Dunning2014}. No contexto IoT, os BDSTs ocupam uma posição central no fluxo de
dados de sensor à percepção que está mudando rapidamente como percebemos e entendemos o
mundo~\cite{Dunning2014}.
...
Ele usa o TsFile, um formato de arquivo colunar personalizado que armazena dados em blocos
...
Log-Structured Merge Tree (LSM), trocando garantias estritas de durabilidade por alta vazão
...
dados cresce~\cite{wang2023iotdb}.
...
pela sobrecarga do runtime Go~\cite{naqvi2017influxdb}.

\textbf{TimescaleDB} estende o PostgreSQL com otimizações para séries temporais~\cite{Ramakrishnan2003}.
...
A literatura de benchmarks categoriza seus instrumentos em vários grupos~\cite{Ramakrishnan2003}.
...
de banco de dados. Como demonstrado nos trabalhos relacionados revisados neste estudo, apenas
uma minoria das avaliações comparativas publicadas fornece os scripts e configurações exatas
necessárias para replicação independente~\cite{pereira2023mulletbench}. Isso limita a capacidade
