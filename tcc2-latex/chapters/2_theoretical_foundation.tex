...
software designed to assist in the maintenance and use of large data collections~\cite{Ramakrishnan2003}.
The need for such systems has grown rapidly, as the alternative --- storing data in flat files
...
that even non-specialist users can understand the stored content~\cite{Ramakrishnan2003}.
...
unique tuples, guaranteeing no duplicate records. Integrity constraints --- conditions specified
...
Distributed databases (DDBs) represent the convergence of two seemingly opposing technologies:
database systems and computer networks~\cite{Ozsu2011}. While traditional database systems
promote centralization for control and integrity, computer networks encourage distribution and
decentralization. The technology of DDBs synthesizes these concepts, demonstrating that data
integration does not necessarily imply physical centralization.
...
distributed over a computer network~\cite{Ozsu2011}. The distributed DBMS (DDBMS) is the
...
and multi-database systems~\cite{Ozsu2011}. In client/server systems, the functions are
...
data, sensor data, events, clicks, and many other types of analytical data~\cite{InfluxDataTimeSeries}.
Figure~\ref{fig:temporal_data} illustrates a time series of meteorological measurements.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\columnwidth]{figures/dado_temporal}
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\columnwidth]{figures/registro_temporal}
...
timestamped record capturing atmospheric pressure, temperature, humidity, and other readings
...
generated every second~\cite{Dunning2014}. This explosion in data volume demands new approaches
...
efficient~\cite{Dunning2014}. This efficiency is achieved through storage strategies that group
...
and the need for extremely low latency make high-performance TSDBs essential~\cite{Dunning2014}.
In the IoT context, TSDBs occupy a central position in the sensor-to-insight data flow that is
rapidly changing how we perceive and understand the world~\cite{Dunning2014}.
...
\textbf{Apache IoTDB}~\cite{wang2023iotdb} is purpose-built for the IoT domain. It uses TsFile,
...
Log-Structured Merge Tree (LSM) approach, trading strict durability guarantees for high
...
advantageous as the dataset grows~\cite{wang2023iotdb}.
...
memory is dominated by the TSM cache, the series index, and Go runtime overhead~\cite{naqvi2017influxdb}.

\textbf{TimescaleDB} extends PostgreSQL with time-series optimizations~\cite{Ramakrishnan2003}.
...
The benchmark literature categorizes its instruments in several groups~\cite{Ramakrishnan2003}.
...
As demonstrated in the related works reviewed in this study, only a minority of published
comparative evaluations provide the scripts and exact configurations needed for independent
replication~\cite{pereira2023mulletbench}. This limits the ability to validate results,
